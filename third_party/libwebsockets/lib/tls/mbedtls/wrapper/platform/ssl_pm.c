...
int ssl_pm_new(SSL *ssl)
{
    ...
    if (TLS_ANY_VERSION != ssl->version) {
        ...
        // NOTE: mbedtls adds 'ssl_conf_version_check()' since v3.1.0, and
        // mbedtls only enable TLS 1.2 by default. So the 'min_tls_version' and
        // 'max_tls_version' must be '0x303', or 'mbedtls_ssl_setup' will be
        // failed.
