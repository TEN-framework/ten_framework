...
def __collect_manifest_tents(directory) -> list[str]:
    ...
    for root, dirs, files in os.walk(directory, followlinks=True):
        for dir in dirs:
            if os.path.exists(os.path.join(directory, dir, MANIFEST_JSON_TENT_FILE)):
...
def update_tman_version(
    log_level: int, year: str, year_month: str, repo_base_dir: str, git_version: str
):
...
def collect_ten_system_packages_and_update_version(
    ...
    # Collect manifest files for ten_runtime and all corresponding system packages(python & go bindings).
    manifest_files = [
        # ten_runtime
        os.path.join(
            repo_base_dir,
    ...
    for manifest_file in manifest_files:
        update_version_in_manifest_json_file(log_level, git_version, manifest_file)
...
def update_package_dependencies(log_level, repo_base_dir, git_version, dependencies):
    ...
    for root, dirs, files in os.walk(packages_dir_path, followlinks=True):
        for dir in dirs:
            manifests += __collect_manifests(os.path.join(packages_dir_path, dir))
            manifests += __collect_manifest_tents(os.path.join(packages_dir_path, dir))
