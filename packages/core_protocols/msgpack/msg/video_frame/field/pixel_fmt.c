...
void ten_msgpack_video_frame_pixel_fmt_serialize(ten_msg_t *self,
                                                 msgpack_packer *pck) {
  TEN_ASSERT(self && ten_raw_msg_check_integrity(self) && pck,
             "Invalid argument.");

  ten_video_frame_t *video_frame = (ten_video_frame_t *)self;
  ten_msgpack_value_serialize(&video_frame->pixel_fmt, pck);
}

bool ten_msgpack_video_frame_pixel_fmt_deserialize(ten_msg_t *self,
  ...
  if (rc == MSGPACK_UNPACK_SUCCESS) {
    if (MSGPACK_DATA_TYPE == MSGPACK_OBJECT_POSITIVE_INTEGER) {
      ten_msgpack_value_deserialize_inplace(
          &((ten_video_frame_t *)self)->pixel_fmt, unpacker, unpacked);
