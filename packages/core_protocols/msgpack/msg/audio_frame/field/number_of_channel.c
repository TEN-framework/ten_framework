...
void ten_msgpack_audio_frame_number_of_channel_serialize(ten_msg_t *self,
                                                         msgpack_packer *pck) {
  TEN_ASSERT(self && ten_raw_msg_check_integrity(self) && pck,
             "Invalid argument.");

  ten_audio_frame_t *audio_frame = (ten_audio_frame_t *)self;
  ten_msgpack_value_serialize(&audio_frame->number_of_channel, pck);
}

bool ten_msgpack_audio_frame_number_of_channel_deserialize(
  ...
  return ten_msgpack_value_deserialize_inplace(
      &((ten_audio_frame_t *)self)->number_of_channel, unpacker, unpacked);
}
