...
def test_failed_to_create_extension_go():
    ...
    if sys.platform == "win32":
        print("test_failed_to_create_extension_go doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path,
            "failed_to_create_extension_go_app/ten_packages/system/ten_runtime/lib",
        )
    else:
        # client depends on some libraries in the TEN app.
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                "failed_to_create_extension_go_app/ten_packages/system/ten_runtime/lib/libasan.so",
            )
    ...
    client_cmd = os.path.join(
        base_path, "failed_to_create_extension_go_app_client"
    )
    ...
    if not is_started:
        print(
            "The failed_to_create_extension_go is not started after 10 seconds."
        )

        server.kill()
        exit_code = server.wait()
        print("The exit code of failed_to_create_extension_go: ", exit_code)
