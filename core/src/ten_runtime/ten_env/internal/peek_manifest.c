...
static ten_env_peek_manifest_sync_context_t *
ten_env_peek_manifest_sync_context_create(void) {
  ten_env_peek_manifest_sync_context_t *context =
      TEN_MALLOC(sizeof(ten_env_peek_manifest_sync_context_t));
...
static void ten_env_peek_manifest_sync_context_destroy(
    ten_env_peek_manifest_sync_context_t *self) {
...
static void ten_app_peek_manifest_sync_cb(ten_app_t *app, ten_value_t *res,
                                          void *cb_data) {
  TEN_ASSERT(app && ten_app_check_integrity(app, true), "Should not happen.");

  ten_env_peek_manifest_sync_context_t *context = cb_data;
...
static ten_env_peek_manifest_async_context_t *
ten_env_peek_manifest_async_context_create(ten_env_t *ten_env,
                                           ten_env_peek_manifest_async_cb_t cb,
                                           void *cb_data) {
  ten_env_peek_manifest_async_context_t *context =
      TEN_MALLOC(sizeof(ten_env_peek_manifest_async_context_t));
  TEN_ASSERT(context, "Failed to allocate memory.");
...
static void ten_env_peek_manifest_async_context_destroy(
    ten_env_peek_manifest_async_context_t *self) {
  TEN_ASSERT(self, "Invalid argument.");

  TEN_FREE(self);
}

static void ten_extension_peek_manifest_async_cb(ten_extension_t *extension,
                                                 ten_value_t *res,
                                                 void *cb_data,
                                                 ten_error_t *err) {
  ...
  ten_env_peek_manifest_async_context_t *context = cb_data;
  TEN_ASSERT(context, "Should not happen.");
  ...
  ten_env_peek_manifest_async_context_destroy(context);
}

static void ten_extension_group_peek_manifest_async_cb(
...
static void ten_env_peek_manifest_done_task(TEN_UNUSED void *from, void *arg) {
  ten_env_peek_manifest_async_context_t *context = arg;
  TEN_ASSERT(context, "Should not happen.");
...
static void ten_app_peek_manifest_async_cb_go_back_to_extension(
  ...
  ten_runloop_post_task_tail(
      ...
      ten_env_peek_manifest_done_task, NULL, context);
}

static void ten_app_peek_manifest_async_cb_go_back_to_extension_group(
    ten_app_t *app, ten_value_t *res, void *cb_data) {
  TEN_ASSERT(app && ten_app_check_integrity(app, true), "Should not happen.");
...
static void ten_app_peek_manifest_async_cb(ten_app_t *app, ten_value_t *res,
                                           void *cb_data) {
  TEN_ASSERT(app && ten_app_check_integrity(app, true), "Should not happen.");
...
ten_value_t *ten_env_peek_manifest(ten_env_t *self, const char *path,
                                   ten_error_t *err) {
    ...
    case TEN_ENV_ATTACH_TO_EXTENSION: {
      ...
      switch (level) {
        case TEN_METADATA_LEVEL_EXTENSION:
          res = ten_extension_peek_manifest(extension, *p_path, err);
          break;
        ...
        case TEN_METADATA_LEVEL_APP: {
          ...
          // thread-check: Access the app's manifest from an extension, that is,
          ...
          if (ten_app_thread_call_by_me(app)) {
            res = ten_app_peek_manifest(app, path);
          } else {
            ten_env_peek_manifest_sync_context_t *context =
                ten_env_peek_manifest_sync_context_create();
            TEN_ASSERT(context, "Should not happen.");

            ten_app_peek_manifest_async(app, path,
                                        ten_app_peek_manifest_sync_cb, context);

            ten_event_wait(context->completed, -1);
            res = context->res;

            ten_env_peek_manifest_sync_context_destroy(context);
          }
          break;
        }
    ...
    case TEN_ENV_ATTACH_TO_EXTENSION_GROUP: {
      ...
      switch (level) {
        ...
        case TEN_METADATA_LEVEL_APP: {
          ...
          // TEN_NOLINTNEXTLINE(thread-check):
          // thread-check: Access the app's manifest from an extension group,
  ...
  if (!res) {
    if (err) {
      ten_error_set(err, TEN_ERRNO_GENERIC, "Failed to find manifest: %s",
                    path);
    }
  }
...
bool ten_env_peek_manifest_async(ten_env_t *self, const char *path,
                                 ten_env_peek_manifest_async_cb_t cb,
  ...
  ten_env_peek_manifest_async_context_t *context =
      ten_env_peek_manifest_async_context_create(self, cb, cb_data);
  TEN_ASSERT(context, "Should not happen.");
  ...
  switch (self->attach_to) {
    case TEN_ENV_ATTACH_TO_EXTENSION: {
      ...
      switch (level) {
        case TEN_METADATA_LEVEL_EXTENSION:
          return ten_extension_peek_manifest_async(
              extension, path, ten_extension_peek_manifest_async_cb, context,
              err);
          break;
        ...
        case TEN_METADATA_LEVEL_EXTENSION_GROUP: {
          ...
          ten_extension_group_peek_manifest_async(
              extension_group, path, ten_extension_group_peek_manifest_async_cb,
              context);
          break;
        }
        ...
        case TEN_METADATA_LEVEL_APP: {
          ten_app_t *app = extension->extension_context->engine->app;
          // TEN_NOLINTNEXTLINE(thread-check):
          // thread-check: Access the app's manifest from an extension, that
          ...
          ten_app_peek_manifest_async(
              app, path, ten_app_peek_manifest_async_cb_go_back_to_extension,
              context);
          break;
        }
    ...
    case TEN_ENV_ATTACH_TO_EXTENSION_GROUP: {
      ...
      switch (level) {
        ...
        case TEN_METADATA_LEVEL_APP: {
          ...
          ten_app_peek_manifest_async(
              app, path,
              ten_app_peek_manifest_async_cb_go_back_to_extension_group,
              context);
          break;
        }
    ...
    case TEN_ENV_ATTACH_TO_APP: {
      ...
      switch (level) {
        case TEN_METADATA_LEVEL_APP:
          ten_app_peek_manifest_async(app, path, ten_app_peek_manifest_async_cb,
                                      context);
          break;
