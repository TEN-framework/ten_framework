...
ten_msg_conversion_t *ten_msg_conversion_create(const char *msg_name) {
  TEN_ASSERT(msg_name, "Invalid argument.");

  ten_msg_conversion_t *self =
      (ten_msg_conversion_t *)TEN_MALLOC(sizeof(ten_msg_conversion_t));
  ...
  self->msg_and_result_conversion_operation = NULL;

  return self;
}

void ten_msg_conversion_destroy(ten_msg_conversion_t *self) {
  TEN_ASSERT(self && ten_msg_conversion_check_integrity(self),
             "Should not happen.");

  ten_string_deinit(&self->msg_name);
  ten_loc_deinit(&self->src_loc);
  if (self->msg_and_result_conversion_operation) {
    ten_msg_and_result_conversion_operation_destroy(
        self->msg_and_result_conversion_operation);
  }

  TEN_FREE(self);
}

static bool ten_msg_conversion_is_equal(ten_msg_conversion_t *a,
                                        ten_msg_conversion_t *b) {
  TEN_ASSERT(a && ten_msg_conversion_check_integrity(a), "Should not happen.");
  TEN_ASSERT(b && ten_msg_conversion_check_integrity(b), "Should not happen.");
...
static bool ten_msg_conversion_can_match_msg(ten_msg_conversion_t *self,
...
bool ten_msg_conversion_merge(ten_list_t *msg_conversions,
                              ten_msg_conversion_t *new_msg_conversion,
                              ten_error_t *err) {
  TEN_ASSERT(msg_conversions && ten_list_check_integrity(msg_conversions),
             "Should not happen.");
  TEN_ASSERT(new_msg_conversion &&
                 ten_msg_conversion_check_integrity(new_msg_conversion),
             "Should not happen.");
  ...
  ten_list_foreach (msg_conversions, iter) {
    ...
    if (ten_msg_conversion_is_equal(msg_conversion, new_msg_conversion)) {
      ...
      ten_msg_conversion_destroy(new_msg_conversion);
      return false;
    }
  }
  ...
  ten_list_push_ptr_back(
      ...
      (ten_ptr_listnode_destroy_func_t)ten_msg_conversion_destroy);
...
bool ten_extension_convert_msg(ten_extension_t *self, ten_shared_ptr_t *msg,
  ...
  TEN_ASSERT(ten_list_check_integrity(&self->extension_info->msg_conversions),
             "Should not happen.");

  ten_list_foreach (&self->extension_info->msg_conversions, iter) {
    ten_msg_conversion_t *msg_conversions = ten_ptr_listnode_get(iter.node);
    TEN_ASSERT(
        msg_conversions && ten_msg_conversion_check_integrity(msg_conversions),
        "Should not happen.");
    ...
    if (ten_msg_conversion_can_match_msg(msg_conversions, msg)) {
      if (msg_conversions->msg_and_result_conversion_operation) {
        ten_msg_conversion_operation_t *msg_conversion =
            msg_conversions->msg_and_result_conversion_operation->msg_operation;
        TEN_ASSERT(msg_conversion, "Invalid argument.");

        // Perform the message conversions.
        ten_shared_ptr_t *new_msg =
            ten_msg_conversion_operation_convert(msg_conversion, msg, err);
        ...
        if (new_msg) {
          ...
          ten_list_push_ptr_back(
              ...
              ten_msg_and_result_conversion_create(
                  new_msg, msg_conversions->msg_and_result_conversion_operation
                               ->result_operation),
              (ten_ptr_listnode_destroy_func_t)
                  ten_msg_and_result_conversion_destroy);
  ...
  if (ten_list_is_empty(result)) {
    ten_list_push_ptr_back(
        result, ten_msg_and_result_conversion_create(msg, NULL),
...
static ten_msg_conversion_t *ten_msg_conversion_from_json_internal(
  ...
  ten_msg_conversion_t *self = ten_msg_conversion_create(original_cmd_name);

  ten_loc_init_from_loc(&self->src_loc, src_loc);

  TEN_ASSERT(ten_json_is_object(json), "Should not happen.");

  ten_msg_and_result_conversion_operation_t *msg_result_operation =
      ten_msg_and_result_conversion_operation_from_json(json, err);
  TEN_ASSERT(msg_result_operation, "Should not happen.");
  if (!msg_result_operation) {
    ten_msg_conversion_destroy(self);
    return NULL;
  }

  self->msg_and_result_conversion_operation = msg_result_operation;

  return self;
}

ten_msg_conversion_t *ten_msg_conversion_from_json(
...
static ten_msg_conversion_t *ten_msg_conversion_from_value_internal(
    ten_value_t *value, ten_loc_t *src_loc, const char *cmd_name,
    ten_error_t *err) {
  TEN_ASSERT(value && cmd_name, "Should not happen.");

  ten_msg_conversion_t *self = ten_msg_conversion_create(cmd_name);
  ...
  ten_msg_and_result_conversion_operation_t *msg_result_operation =
      ten_msg_and_result_conversion_operation_from_value(value, err);
  TEN_ASSERT(msg_result_operation, "Should not happen.");
  if (!msg_result_operation) {
    ten_msg_conversion_destroy(self);
    return NULL;
...
ten_msg_conversion_t *ten_msg_conversion_from_value(
