...
void ten_extension_context_on_all_extensions_in_extension_group_are_stopped(
    void *self_, void *arg) {
  ...
  TEN_LOGD("[%s] Engine is notified that %s is_stopped",
           ten_engine_get_name(self->engine),
           ten_string_get_raw_str(&extension_group->name));

  self->extension_threads_cnt_of_all_extensions_stopped++;

  // TODO(Wei): At present, we only check whether all extension threads in the
  // same TEN app has reached the 'stopped' state. And if this condition is met,
  // the engine will enable all the extension threads (in the belonging TEN app)
  // proceed to the 'deinit' stage.
  //
  // If, in the future, we need to ensure that all extension threads in _all_
  // the TEN app have reached the 'stopped' state before entering into the
  // 'deinit' stage, this location is the right place to add more logic about
  // this.

  if (self->extension_threads_cnt_of_all_extensions_stopped ==
      ten_list_size(&self->extension_threads)) {
    TEN_LOGD(
        "[%s] All extension threads enter 'all extensions are stopped' state.",
        ten_engine_get_name(self->engine));
    ...
    ten_list_foreach (&self->extension_threads, iter) {
      ...
      ten_runloop_post_task_tail(
          ...
          ten_extension_thread_pre_close, thread, NULL);
    }
  }
}
