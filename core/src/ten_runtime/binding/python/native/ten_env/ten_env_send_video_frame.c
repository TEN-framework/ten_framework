...
ten_env_notify_send_video_frame_info_create(ten_shared_ptr_t *c_cmd) {
...
static void ten_env_proxy_notify_send_video_frame(ten_env_t *ten_env,
  ...
  ten_env_notify_send_video_frame_info_t *notify_info = user_data;

  ten_env_send_video_frame(ten_env, notify_info->c_audio_frame, NULL, NULL,
                           NULL);
...
PyObject *ten_py_ten_env_send_video_frame(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!", ten_py_video_frame_py_type(),
                        &py_video_frame)) {
    success = false;
    ten_py_raise_py_type_error_exception(
        "Invalid argument type when send video frame.");
    goto done;
  }
