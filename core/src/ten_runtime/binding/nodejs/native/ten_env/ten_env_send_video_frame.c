...
typedef struct ten_nodejs_send_video_frame_callback_call_info_t {
  ten_nodejs_tsfn_t *js_cb;
  ten_error_t *error;
} ten_nodejs_send_video_frame_callback_call_info_t;
...
static ten_nodejs_send_video_frame_callback_call_info_t *
ten_nodejs_send_video_frame_callback_call_info_create(ten_nodejs_tsfn_t *js_cb,
                                                      ten_error_t *error) {
  ten_nodejs_send_video_frame_callback_call_info_t *info =
      TEN_MALLOC(sizeof(ten_nodejs_send_video_frame_callback_call_info_t));
  TEN_ASSERT(info, "Failed to allocate memory.");
...
static void ten_nodejs_send_video_frame_callback_call_info_destroy(
    ten_nodejs_send_video_frame_callback_call_info_t *info) {
  TEN_ASSERT(info, "Invalid argument.");
...
static void tsfn_proxy_send_video_frame_callback(napi_env env, napi_value js_cb,
                                                 void *context,
                                                 void *video_frame) {
  ten_nodejs_send_video_frame_callback_call_info_t *info = video_frame;
  ...
  ten_nodejs_send_video_frame_callback_call_info_destroy(info);
}
...
static void proxy_send_video_frame_callback(ten_env_t *ten_env,
  ...
  ten_env_notify_send_video_frame_info_t *info = user_video_frame;
  TEN_ASSERT(info, "Should not happen.");
  ...
  ten_nodejs_send_video_frame_callback_call_info_t *call_info =
      ten_nodejs_send_video_frame_callback_call_info_create(js_cb,
                                                            cloned_error);
  ...
  ten_env_notify_send_video_frame_info_destroy(info);
}
...
static void ten_env_proxy_notify_send_video_frame(ten_env_t *ten_env,
  ...
  bool rc =
      ten_env_send_video_frame(ten_env, info->c_video_frame,
                               proxy_send_video_frame_callback, info, &err);
  if (!rc) {
    proxy_send_video_frame_callback(ten_env, NULL, info, &err);
  }
