...
pub fn update_graph_connections_all_fields(
    ...
    let found_graph = find_and_update_graph(
        predefined_graphs,
        graph_name,
        connections_to_add,
    ...
    // If no graph was found, we still need to write back the property file
    if !found_graph {
        // This is a no-op case, but we still need to write back the file
    }
...
fn update_destinations(
    ...
    for conn_dest in conn_dests.iter_mut() {
        ...
        for modify_dest in &modify_flow.dest {
            ...
            break; // Found matching destination, no need to check more
        }
    }
}
